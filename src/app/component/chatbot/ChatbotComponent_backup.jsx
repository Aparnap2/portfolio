...
const ChatbotComponent = ({
  ...
  return (
    <div className={`fixed z-50 flex items-center justify-center ${isMobile ? 'inset-0' : 'inset-0 p-4'
      }`} role="dialog" aria-modal="true" aria-label="AI Business Assistant Chat">
      <div
        className={`relative bg-gray-900/95 backdrop-blur-xl border border-gray-600/70 shadow-2xl overflow-hidden transition-all duration-300 ${isMobile ? 'rounded-none' : 'rounded-2xl'
      ...
      >
            ...
            {!isMinimized && (
              <div>
                ...
                <div className="flex items-center space-x-2">
                  <p className="text-xs text-gray-300" id="chatbot-status">Helping you explore AI automation solutions</p>
                  {connectionStatus !== 'connected' && (
                    <div className={`flex items-center space-x-1 text-xs ${connectionStatus === 'connecting' ? 'text-yellow-300' : 'text-red-300'
            ...
            <nav className="flex items-center space-x-1" role="navigation" aria-label="Chat controls">
              ...
              {error && canRetry && (
                <button
                  ...
                  className={`p-2 text-gray-300 hover:text-blue-300 rounded-full hover:bg-blue-600/20 focus:outline-none focus:ring-2 focus:ring-blue-400/70 focus:ring-offset-1 focus:ring-offset-gray-900/50 ${prefersReducedMotion ? '' : 'transition-all duration-200 hover:scale-110'
                    }`}
              ...
              {!isMobile && (
                <button
                  ...
                  className={`p-2 text-gray-300 hover:text-white rounded-full hover:bg-gray-700/50 focus:outline-none focus:ring-2 focus:ring-gray-400/70 focus:ring-offset-1 focus:ring-offset-gray-900/50 ${prefersReducedMotion ? '' : 'transition-all duration-200 hover:scale-110'
                    }`}
              ...
              {!isMobile && onMinimize && (
                <button
                  ...
                  className={`p-2 text-gray-300 hover:text-yellow-300 rounded-full hover:bg-yellow-600/20 focus:outline-none focus:ring-2 focus:ring-yellow-400/70 focus:ring-offset-1 focus:ring-offset-gray-900/50 ${prefersReducedMotion ? '' : 'transition-all duration-200 hover:scale-110'
                    }`}
              ...
              <button
                ...
                className={`p-2 text-gray-300 hover:text-red-300 rounded-full hover:bg-red-600/20 focus:outline-none focus:ring-2 focus:ring-red-400/70 focus:ring-offset-1 focus:ring-offset-gray-900/50 ${prefersReducedMotion ? '' : 'transition-all duration-200 hover:scale-110'
                  }`}
        ...
        {!isMinimized && (
          ...
          >
            ...
            {/* Error display with better contrast */}
            {error && (
              <div className="bg-red-900/40 border border-red-600/60 rounded-xl p-4 mb-4" role="alert" aria-live="assertive">
                <div className="flex items-start space-x-3">
                  ...
                  <div className="flex-1">
                    ...
                    {canRetry && (
                      <button
                        onClick={handleRetry}
                        className="mt-2 text-sm text-blue-300 hover:text-blue-200 underline focus:outline-none focus:ring-2 focus:ring-blue-400/70 rounded"
                      >
                        Try again
                      </button>
                    )}
        ...
        {!isMinimized && (
          <footer className={`absolute bottom-0 left-0 right-0 bg-gradient-to-t from-gray-900/95 to-transparent backdrop-blur-sm ${isMobile ? 'p-3' : 'p-4'
            }`} role="contentinfo" aria-label="Message input area">
            <form
            ...
            >
              <div className="flex-1 relative">
                ...
                <textarea
                  ...
                  className={`w-full bg-gray-800/80 border border-gray-600/70 text-gray-100 rounded-xl py-3 px-4 pr-12 focus:outline-none focus:ring-2 focus:ring-blue-400/70 focus:border-blue-400/70 transition-all duration-200 resize-none overflow-y-auto backdrop-blur-sm ${isMobile ? 'min-h-[44px] max-h-24 text-[16px]' : 'min-h-[48px] max-h-32'
                    }`}
                  rows="1"
                  disabled={isLoading}
                ...
                {!isLoading ? (
                  <button
                    type="submit"
                    disabled={!input.trim()}
                    className={`absolute right-2 bottom-2 p-2 rounded-lg transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-offset-1 focus:ring-offset-gray-900/50 ${input.trim()
                        ? 'bg-gradient-to-r from-blue-500 to-purple-500 text-white hover:opacity-90 hover:scale-105 shadow-lg focus:ring-blue-400/70 active:scale-95'
                  ...
                  <button
                    ...
                    className="absolute right-2 bottom-2 p-2 rounded-lg bg-red-600/80 hover:bg-red-600 text-white transition-all duration-200 hover:scale-105 shadow-lg focus:outline-none focus:ring-2 focus:ring-red-400/70 focus:ring-offset-1 focus:ring-offset-gray-900/50 active:scale-95"
            ...
            {/* Footer with contact links - improved contrast */}
            <div className={`mt-3 flex gap-2 ${isMobile ? 'flex-col items-center' : 'flex-row items-center justify-between'
              ...
              <div className="flex items-center space-x-4" role="navigation" aria-label="Contact options">
                <a
                  href="mailto:softservicesinc.portfolio@gmail.com"
                  className="text-xs text-blue-300 hover:text-blue-200 flex items-center space-x-1 transition-colors hover:underline focus:outline-none focus:ring-2 focus:ring-blue-400/70 rounded"
                  title="Email Aparna"
                >
                ...
                <a
                  ...
                  className="text-xs text-purple-300 hover:text-purple-200 flex items-center space-x-1 transition-colors hover:underline focus:outline-none focus:ring-2 focus:ring-purple-400/70 rounded"
